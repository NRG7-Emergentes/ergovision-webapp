import { Component, ChangeDetectionStrategy, OnInit, signal, inject } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import {ZardProgressBarComponent} from '@shared/components/progress-bar/progress-bar.component';
import {HistoryService} from '@app/history/services/history.service';
import {SessionDetail} from '@app/history/models/session.model';
import {toast} from 'ngx-sonner';
import {ZardButtonComponent} from '@shared/components/button/button.component';

@Component({
  selector: 'app-session-page',
  template: `
    <div class="container mx-auto py-8 px-4 sm:px-6 lg:px-8">

      <div class="mb-8">
        <h1 class="text-3xl sm:text-4xl font-bold text-foreground tracking-tight">Session Details</h1>
        <p class="text-muted-foreground mt-2">Overview of the monitoring session</p>
      </div>

      @if (isLoading()) {
        <div class="flex items-center justify-center py-12">
          <div class="text-muted-foreground">Loading session data...</div>
        </div>
      } @else if (session()) {
        <div class="grid grid-cols-1 sm:grid-cols-2  gap-4 ">
          <div class="bg-card border p-6 rounded-lg shadow-sm text-card-foreground w-full flex items-center justify-between">
            <span class="text-md font-medium tracking-widest text-muted-foreground text-xl"> Date (local timezone) </span>
            <div class="bg-background border rounded-full px-4 py-2 ">
              <span class="font-bold text-2xl"> {{ session()!.date }} </span>
            </div>
          </div>
          <div class="bg-card border p-6 rounded-lg shadow-sm text-card-foreground w-full flex items-center justify-between">
            <span class="text-md font-medium tracking-widest text-muted-foreground text-xl"> Session Time </span>
            <div class="bg-background border rounded-full px-4 py-2 ">
              <span class="font-bold text-2xl"> {{ session()!.duration }} </span>
            </div>
          </div>
          <div class="bg-card block border p-6 rounded-lg shadow-sm text-card-foreground w-full">
            <p class="text-xl font-bold mb-4"> Posture</p>
            <div class="flex flex-col gap-4">
              <div>
                <div class="mb-2 flex justify-between items-center">
                  <span class="text-md font-medium tracking-widest"> Good Posture </span>
                  <span> {{ session()!.posture.goodPercent }}% </span>
                </div>
                <z-progress-bar [progress]="session()!.posture.goodPercent"  zType="constructive"></z-progress-bar>
                <div class="mt-2">
                  <span class="text-muted-foreground text-sm"> Time in Good Posture: {{ session()!.posture.goodTime }}</span>
                </div>
              </div>
              <div>
                <div class="mb-2 flex justify-between items-center">
                  <span class="text-md font-medium tracking-widest"> Bad Posture </span>
                  <span> {{ session()!.posture.badPercent }}% </span>
                </div>
                <z-progress-bar [progress]="session()!.posture.badPercent"  zType="destructive"></z-progress-bar>
                <div class="mt-2">
                  <span class="text-muted-foreground text-sm"> Time in Bad Posture: {{ session()!.posture.badTime }}</span>
                </div>
              </div>
            </div>

          </div>
          <div class="bg-card block border p-6 rounded-lg shadow-sm text-card-foreground w-full">
            <p class="text-xl font-bold mb-4"> Pauses</p>
            <div class="flex flex-col gap-4  mb-2 ">
              <div class="p-4 bg-secondary rounded-lg flex items-center justify-between">
                <span class="font-medium tracking-widest">Number of Pauses:</span>
                <span class="text-muted-foreground tracking-widest">{{ session()!.pauses.count }} Pauses</span>
              </div>
              <div class="p-4 bg-secondary rounded-lg flex items-center justify-between">
                <span class="font-medium tracking-widest">Average Pause Time:</span>
                <span class="text-muted-foreground tracking-widest">{{ session()!.pauses.avgTime }}</span>
              </div>
              <div class="p-4 bg-secondary rounded-lg flex items-center justify-between">
                <span class="font-medium tracking-widest">Total Pause Time:</span>
                <span class="text-muted-foreground tracking-widest">{{ session()!.pauses.totalTime }}</span>
              </div>
            </div>
            <span class="text-muted-foreground text-sm"> Note: pauses are detected when inactivity > threshold. </span>
          </div>
        </div>
        
        <div class="mt-6">
          <button z-button zType="secondary" (click)="goBack()">
            <i class="icon-arrow-left"></i>
            Back to History
          </button>
        </div>
      } @else {
        <div class="bg-card border p-6 rounded-lg shadow-sm text-card-foreground">
          <p class="text-muted-foreground">Session not found.</p>
          <button z-button zType="secondary" class="mt-4" (click)="goBack()">
            Back to History
          </button>
        </div>
      }
    </div>
  `,
  styles: [``],
  imports: [
    ZardProgressBarComponent,
    ZardButtonComponent
  ],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SessionPageComponent implements OnInit {
  id: string | null = null;
  session = signal<SessionDetail | undefined>(undefined);
  isLoading = signal<boolean>(true);

  private route = inject(ActivatedRoute);
  private router = inject(Router);
  private svc = inject(HistoryService);

  ngOnInit(): void {
    this.id = this.route.snapshot.paramMap.get('id');
    if (this.id) {
      this.loadSession(this.id);
    } else {
      this.isLoading.set(false);
    }
  }

  loadSession(id: string): void {
    this.isLoading.set(true);
    this.svc.getSession(id).subscribe({
      next: (sessionData) => {
        this.session.set(sessionData);
        this.isLoading.set(false);
      },
      error: (error) => {
        console.error('Error loading session:', error);
        toast.error('Failed to load session details');
        this.isLoading.set(false);
        this.session.set(undefined);
      }
    });
  }

  goBack(): void {
    this.router.navigate(['/history']);
  }
}
